...
do
        ...
        # # Combine the left and right BNST Streamlines
        # tckedit tracks_5M_BNST_L.tck tracks_5M_BNST_R.tck -force combined_BNST_tracks.tck

        # Refining the combined_BNST_tracks Streamlines with tcksift2
        tcksift2 -proc_mask ndi_5tt_mask.mif \
                -out_mu sift_mu_noACT_1M_BNST_L.txt \
                -out_coeffs sift_coeffs_noACT_1M_BNST_L.txt \
                -nthreads 24 \
                tracks_noACT_1M_BNST_L.tck \
                wmfod_norm.mif \
                sift_noACT_1M_BNST_L.txt

        tcksift2 -proc_mask ndi_5tt_mask.mif \
                -out_mu sift_mu_noACT_1M_BNST_R.txt \
                -out_coeffs sift_coeffs_noACT_1M_BNST_R.txt \
                -nthreads 24 \
                tracks_noACT_1M_BNST_R.tck \
                wmfod_norm.mif \
                sift_noACT_1M_BNST_R.txt
